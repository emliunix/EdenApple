...
exports.default = {
  option: _option2.default, atry: _try2.default, unit: _unit2.default
}; /*
    * Parsec
...
var Genlex = function () {
    ...
    _createClass(Genlex, [{
        ...
        value: function tokens(f) {
            return this.tokenBetweenSpaces(f).optrep().thenLeft(_flowBundle2.default.eos);
        }
    }]);
...
exports.default = {
  genlex: _genlex2.default, token: _token2.default, tokenizer: _tokenizer2.default
}; /*
    * Parsec
    * https://github.com/d-plaindoux/parsec
    *
...
function string(s) {
    return _flowBundle2.default.try(subString(s.length).filter(function (r) {
        return r === s;
    }));
}
...
function sequence() {
    ...
    var current = getParser(args[0]);
    for (var i = 1; i < args.length; i++) {
        var next = getParser(args[i]);
        current = current.then(next).map(function (values) {
...
exports.default = {
  parser: _parser2.default, response: _response2.default
};
//# sourceMappingURL=index.js.map
...
var _option = require('../data/option.js');
...
var Parser = function () {
    ...
    _createClass(Parser, [{
        ...
        key: 'flattenDeep',
        value: function flattenDeep() {
            return this.map(function (self) {
                return _flattenDeep(self);
            });
        }
    }]);
...
var ExtractorBundle = function () {
    ...
    _createClass(ExtractorBundle, [{
        ...
        value: function words() {
            ...
            if (keepSpaces) {
                return _index.F.try(this.word().or(this._wordSeparators())).rep();
            } else {
        ...
        value: function wordsIn(array) {
            ...
            if (keepSpaces) {
                return _index.F.try(this.stringIn(array).or(this._wordSeparators())).rep();
            } else {
...
exports.default = {
  jsonParser: _jsonparser2.default, markdownDocument: _markdownParser2.default, markdownBundle: _markdownBundle2.default, extractorBundle: _extractorBundle2.default, tokenBundle: _tokenBundle2.default
}; /*
    * Parsec
    * https://github.com/d-plaindoux/parsec
    *
...
function pureText() {
    return _index.F.not(stop()).rep().map(function (characters) {
...
function bulletLv1() {
    ...
    .thenRight(formattedSequence()).map(function (someText) {
        return { bullet: { level: 1, content: someText } };
    });
}
...
function bulletLv2() {
    ...
    .then(_index.C.charIn(' \xA0')) // second character of a bullet is space or non-breakable space
    .thenRight(formattedSequence()).map(function (someText) {
        return { bullet: { level: 2, content: someText } };
    });
}
...
function formattedParagraph() {
    return _token2.default.blank().thenRight(formattedSequence(pureText(), stop())).map(function (array) {
...
function rawTextUntilChar(charList) {
    ...
    if (allowVoid) {
        return _index.C.charNotIn(charList).optrep().map(function (characters) {
            return characters.join('');
        });
    } else {
        return _index.C.charNotIn(charList).rep().map(function (characters) {
            return characters.join('');
        });
    }
}
...
function _inQuote() {
    return _index.C.char('"').then(_index.C.notChar('"').rep()).then(_index.C.char('"'));
}
...
function email() {
    ...
    return _inQuote().or(_index.C.charNotIn(illegalCharSet1)).rep() // this mean:   repeat(inQuote or anyCharacter)
    .then(_index.C.char('@')).then(_index.C.charNotIn(illegalCharSet2).rep()).then(_index.C.char('.')).then(_index.C.charNotIn(illegalCharSet2).rep()).flattenDeep().map(function (characters) {
        return { email: characters.join('') };
    });
}

function date() {
    return _index.N.digits.then(_index.C.charIn('-/').thenReturns('-')).then(_index.N.digits).then(_index.C.charIn('-/').thenReturns('-')).then(_index.N.digits).map(_index.F.flattenDeep).map(function (dateValues) {
